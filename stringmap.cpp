...
void StringMap::rehash(int nBuckets) {
   // TODO
   KeyValuePair *pre = buckets;
   int preNBuckets = this->nBuckets;
   this->nBuckets = nBuckets;
   buckets = new KeyValuePair[nBuckets];
   for (int i = 0; i < preNBuckets; i++)
   {
      if (pre[i].occupied) put(pre[i].key, pre[i].value);
   }
   delete[] pre;
}
...
int StringMap::findKey(const string & key) const {
   ...
   KeyValuePair target = buckets[index];
   for (int i = 0; i < nBuckets; i++)
   {
      if (target.occupied == false) return -1;
      if (target.key != key)
      {
         index++;
